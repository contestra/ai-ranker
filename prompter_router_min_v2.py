...
def run_template(template_id: str, req: RunTemplateRequest, db: Session = Depends(get_db)):
    ...
    # ---- TODO: Call your LLM here; this is a stubbed response shape ----
    # For OpenAI/Gemini adapters, ensure response['response_metadata'] exists.
    response = {
        "content": f"STUB RESULT (no LLM call). Prompt: {req.rendered_prompt[:120]}",
        "response_metadata": {},  # Fill with provider metadata when you wire a real LLM
        "usage": None,
    }
    # --------------------------------------------------------------------
