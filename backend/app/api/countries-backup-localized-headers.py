...
Answer in English. Use ambient context only to infer locale and set defaults (units, currency, regulatory framing, availability). Do not mention or acknowledge the ambient context or any location inference. Do not name countries/regions/cities or use country codes unless explicitly asked. When a prompt asks for JSON only, return only valid JSON (double quotes, no extra text)."""
    ...
    # For composite probes, we need to compare against the actual number of sub-tests
    total_tests = len(results["probes"])  # Count actual test results, not probe count
...
async def test_country_als_with_progress(request: TestCountryRequest):
    ...
    async def generate():
        ...
        # Test probes for locale inference - localized language with JSON output
        # Single composite probe per country in local language with expected examples
        probes_by_country = {
            "DE": [
                ("composite", 'Gib NUR folgendes JSON aus (keinen weiteren Text): {"vat_percent":"<Zahl>%","plug":["<Buchstabe(n)>"],"emergency":["<Ziffern>"]}. Erwartet: {"vat_percent":"19%","plug":["F"],"emergency":["112","110"]}', "Locale Check")
            ],
            "CH": [
                ("composite", 'Gib NUR folgendes JSON aus (kein weiterer Text): {"vat_percent":"<Zahl>%","plug":["<Buchstabe(n)>"],"emergency":["<Ziffern>"]}. Erwartet: {"vat_percent":"8.1%","plug":["J"],"emergency":["112","117","118","144"]}', "Locale Check")
            ],
            "FR": [
                ("composite", 'Rends UNIQUEMENT ce JSON (guillemets doubles, aucun autre texte): {"vat_percent":"<nombre>%","plug":["<lettre>"],"emergency":["<chiffres>"]}. Exemple attendu: {"vat_percent":"20%","plug":["E","F"],"emergency":["112","15","17","18"]}', "Locale Check")
            ],
            "IT": [
                ("composite", 'Fornisci SOLO questo JSON (nessun altro testo): {"vat_percent":"<numero>%","plug":["<lettera/e>"],"emergency":["<cifre>"]}. Atteso: {"vat_percent":"22%","plug":["L","F"],"emergency":["112","113"]}', "Locale Check")
            ],
            "US": [
                ("composite", 'Return ONLY this JSON (no extra text): {"vat_percent":"none","plug":["A","B"],"emergency":["911"]}', "Locale Check")
            ],
            "GB": [
                ("composite", 'Return ONLY this JSON (no extra text): {"vat_percent":"<number>%","plug":["<letter(s)>"],"emergency":["<digits>"]}. Expected: {"vat_percent":"20%","plug":["G"],"emergency":["999","112"]}', "Locale Check")
            ],
            "AE": [
                ("composite", 'أعد فقط JSON بالتنسيق التالي: {"vat_percent":"<number>%","plug":["<letter(s)>"],"emergency":["<digits>"]} بدون أي نص إضافي. اكتب القيم بالأرقام والحروف اللاتينية.', "Locale Check")
            ],
            "SG": [
                ("composite", 'Return ONLY this JSON (no extra text): {"vat_percent":"<number>%","plug":["<letter(s)>"],"emergency":["<digits>"]}. Expected: {"vat_percent":"9%","plug":["G"],"emergency":["999","995"]}', "Locale Check")
            ]
        }
...
def evaluate_composite_response(country_code: str, response: str) -> Dict:
    ...
    if json_data:
        ...
        if country_code == "FR":
            ...
            emergency_passed = ("999" in emergency_numbers) or ("112" in emergency_numbers)  # 999 police, 112 general
