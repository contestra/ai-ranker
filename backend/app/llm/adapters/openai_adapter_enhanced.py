...
def run_openai_with_grounding(
    ...
    *,
    ...
    default_cap = int(os.getenv("OPENAI_GPT5_TOOLS_MAX_OUTPUT_TOKENS", "512"))
    ...
    # Optional provoker when soft-required
    provoker_used = None
    if soft_required:
        provoker_used = (provoker or _default_provoker()).strip()

    msgs = _build_messages(system=system, als=als, prompt=prompt, provoker=provoker_used)
